...
const Sidebar = () => {
  ...
  return (
    <>
      <aside className="w-56 p-4 bg-white border-r lg:flex hidden flex-col fixed top-0 left-0 h-full">
        <Link
          href={"/"}
          ...
          {sidebarLinks.map((link, index) => (
            <li key={index} className="flex">
              <Link
                href={link.href}
                className={cn(
                  "flex items-center gap-3 w-full p-2 rounded-md font-medium text-gray-400 hover:bg-main transition duration-200",
                  {
